...
def load_shelter_data():
    """
    Loads bed-based and room-based shelter risk outputs and tags each row
    with its source so both can be plotted together or filtered separately.
    """
    frames = []
    try:
        bed = pd.read_csv("model results/bed_based_shelters_map.csv").dropna(subset=['LATITUDE', 'LONGITUDE'])
        bed["shelter_type"] = "Bed-Based"
        frames.append(bed)
    except (FileNotFoundError, KeyError):
        pass
 
    try:
        room = pd.read_csv("model results/room_based_shelters_map.csv").dropna(subset=['LATITUDE', 'LONGITUDE'])
        room["shelter_type"] = "Room-Based"
        frames.append(room)
    except (FileNotFoundError, KeyError):
        pass
    ...
    return pd.DataFrame(columns=[
        'SHELTER_ID', 'LOCATION_ADDRESS', 'LOCATION_POSTAL_CODE',
        'LATITUDE', 'LONGITUDE', 'avg_future_risk', 'pressure_risk', 'shelter_type'
    ])

@st.cache_data
def load_encampment_data():
    try:
        return pd.read_csv("model results/okai_high_risk_encampments.csv").dropna(subset=['lat', 'lon'])
    except (FileNotFoundError, KeyError):
        return pd.DataFrame(columns=['cell_id', 'lat', 'lon', 'probability', 'color_code'])
...
encampment_df = load_encampment_data()
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### Additional Risk Layers")
    show_shelters = st.checkbox("Shelters", help="Bed-based and room-based shelters, colored by predicted occupancy pressure")
    show_encampments = st.checkbox("Encampment Growth", help="Grid cells flagged as high-risk for encampment growth")
    ...
    st.markdown(
        ...
        "Period product desert zones are areas where access to menstrual products is limited due to poverty, geography, or the absence of nearby distribution points.<br><br>"
...
<div class="main-header">
    <h1>OKAI Period</h1>
    <p><strong>O</strong>ptimizing for <strong>K</strong>nowledge using <strong>A</strong>rtificial <strong>I</strong>ntelligence</p>
</div>
""", unsafe_allow_html=True)
...
if show_shelters and not shelter_df.empty:
    for _, row in shelter_df.iterrows():
        ...
        <div style="font-family: 'Arial', sans-serif; font-size: 12px; color: #333; min-width: 200px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                <tr><td><b>Type:</b></td><td style="text-align: right;">{row['shelter_type']}</td></tr>
                <tr><td><b>Pressure Risk:</b></td><td style="text-align: right; font-weight: bold;">{row['pressure_risk']}</td></tr>
                <tr><td><b>Avg. Future Risk:</b></td><td style="text-align: right;">{row['avg_future_risk']:.2f}</td></tr>
            </table>
        </div>
        """
...
if show_encampments and not encampment_df.empty:
    for _, row in encampment_df.iterrows():
        ...
        <div style="font-family: 'Arial', sans-serif; font-size: 12px; color: #333; min-width: 180px;">
            <h4 style="margin: 0 0 8px 0; color: #ef4444; border-bottom: 1px solid #ddd; padding-bottom: 4px;">
                Grid Cell {int(row['cell_id'])}
            </h4>
            <table style="width: 100%; border-collapse: collapse;">
                <tr><td><b>Growth Probability:</b></td><td style="text-align: right; font-weight: bold;">{row['probability']:.2f}</td></tr>
            </table>
        </div>
        """
